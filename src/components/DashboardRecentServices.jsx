    ...
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white p-4">
      ...
      <div className="flex flex-col gap-2 mb-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button className="flex items-center gap-2 border border-gray-300 px-3 sm:px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-[5px]">
            <Filter className="size-4 sm:size-5" /> Filtrar
          </button>
          <button className="flex items-center gap-2 border border-gray-300 px-3 sm:px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-[5px]">
            <Eye className="size-4 sm:size-5" /> Visualizar Tudo
          </button>
        </div>
      </div>
      ...
      <div className="w-full overflow-x-auto">
        <table className="w-full min-w-[600px] border-collapse border border-gray-200">
          ...
          <thead className="text-gray-600 text-xs sm:text-sm">
            <tr>
              <th className="py-3 px-2 sm:px-4 text-left">Serviço</th>
              <th className="py-3 px-2 sm:px-4 text-left">Categoria</th>
              <th className="py-3 px-2 sm:px-4 text-left">Valor</th>
          ...
          <tbody className="divide-y divide-gray-200 text-xs sm:text-sm">
            {tableData.map((product) => (
              <tr key={product.id} className="hover:bg-gray-50">
                <td className="py-3 px-2 sm:px-4 flex items-center gap-2 sm:gap-3">
                  <img
                    src={`/img/${product.image}`}
                    className="h-[20px] sm:h-[26px] w-[40px] sm:w-[50px] object-cover rounded-md"
                    alt={product.name}
                  />
                  <div>
                    <p className="font-medium text-gray-800">{product.name}</p>
                    <span className="text-gray-500 text-[10px] sm:text-xs">{product.variants}</span>
                  </div>
                </td>
                <td className="py-3 px-2 sm:px-4 text-gray-500">{product.category}</td>
                <td className="py-3 px-2 sm:px-4 text-gray-500">{product.price}</td>
                <td className="py-3 px-2 sm:px-4">
                  <span
                    className={`px-2 sm:px-3 py-1 text-white text-[10px] sm:text-xs font-semibold rounded-full ${
